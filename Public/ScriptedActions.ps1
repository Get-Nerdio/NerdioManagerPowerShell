...
Function New-NmeScriptedActions {
	...
	[CmdletBinding()]
	Param(
		[Parameter(Mandatory=$true, ValueFromPipelineByPropertyName=$True)][ValidateScript({if ($_.PSObject.TypeNames -contains "NmeCreateScriptedActionRequest"){$true} else{throw " is not a NmeCreateScriptedActionRequest object."}})][Alias("NmePatchScriptedActionRequest")]$NmeCreateScriptedActionRequest
	)
	Set-NmeAuthHeaders
	Try {
...
Function Set-NmeScriptedAction {
	...
	.PARAMETER NmePatchScriptedActionRequest

	Requires an NmePatchScriptedActionRequest object, as generated by the New-NmePatchScriptedActionRequest command.
	...
	Param(
		...
		[Parameter(Mandatory=$true, ValueFromPipelineByPropertyName=$True)][ValidateScript({if ($_.PSObject.TypeNames -contains "NmePatchScriptedActionRequest"){$true} else{throw " is not a NmePatchScriptedActionRequest object."}})][Alias("NmeCreateScriptedActionRequest")]$NmePatchScriptedActionRequest
	)
	Set-NmeAuthHeaders
	Try {
		$json = $NmePatchScriptedActionRequest | ConvertTo-Json -Depth 20
		Write-Debug 'json:'
		Write-Debug $json
