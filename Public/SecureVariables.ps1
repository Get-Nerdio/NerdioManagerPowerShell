...
Function New-NmeSecureVariable {
	...
	[CmdletBinding()]
	Param(
		[Parameter(Mandatory=$true, ValueFromPipelineByPropertyName=$True)][ValidateScript({if ($_.PSObject.TypeNames -contains "NmeCreateOrUpdateSecureVariableRestPayload"){$true} else{throw " is not a NmeCreateOrUpdateSecureVariableRestPayload object."}})]$NmeCreateOrUpdateSecureVariableRestPayload
	)
	Set-NmeAuthHeaders
	Try {
